...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			...
			title: localize('move second sidebar left', "Move Deca Side Bar Left") // Deca renamed "Secondary" Side Bar to "Deca" Side Bar
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move Deca Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "Deca Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
...
class MoveFocusedViewAction extends Action2 {
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "Deca Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "Deca Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
